...
class RecursiveDivision extends Maze {
  ...
  step() {
    switch (this.state) {
      ...
      case this.MAKE_PASSAGE:
        ...
        break;
      default:
        console.error("RECUSIVE DIVISION: Defaulting state, maze.state = ", this.state)
    }
  ...
  makeWall() {
    ...
    this.wallStartCell = {
      x: this.region.x + (this.horizontal ? 0 : Math.floor(this.prng.random(this.region.width - 2))),
      y: this.region.y + (this.horizontal ? Math.floor(this.prng.random(this.region.height - 2)) : 0)
    }
  ...
  makePassage() {
    ...
    let passage = {
      x: this.wallStartCell.x + (this.horizontal ? Math.floor(this.prng.random(this.region.width)) : 0),
      y: this.wallStartCell.y + (this.horizontal ? 0 : Math.floor(this.prng.random(this.region.height)))
    };

    this.removeWall(passage, this.dir);

    let width = this.horizontal ? this.region.width : this.wallStartCell.x - this.region.x + 1
    let height = this.horizontal ? this.wallStartCell.y - this.region.y + 1 : this.region.height
    if (width >= 2 && height >= 2)
      this.stack.push({
        x: this.region.x,
        y: this.region.y,
    ...
    width = this.horizontal ? this.region.width : this.region.x + this.region.width - this.wallStartCell.x - 1
    height = this.horizontal ? this.region.y + this.region.height - this.wallStartCell.y - 1 : this.region.height
