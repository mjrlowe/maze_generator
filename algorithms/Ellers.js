...
class Eller extends Maze {
  ...
  horizontalStep() {
    if (!(this.rowState.cells[this.currentCell.x] === this.rowState.cells[this.currentCell.x + 1]) && (this.currentCell.y === this.xSize-1 || this.prng.random() > 0.5)) {
  ...
  computeVerticals() {
    ...
    for (let setId in this.rowState.sets) {
      ...
      let numberOfCellsToConnect = Math.floor(this.prng.random()*(set.length-1))+1;
  ...
  verticalStep() {
    ...
    if (this.cellsToConnectVertically.length === 0) {
      ...
      this.nextState.add(this.currentCell.x, this.rowState.cells[this.currentCell.x])
