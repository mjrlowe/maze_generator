...
| cellSelectionMethod | Cell selection method for `"growing tree"` algorithm. | An object with one or more of the following properties: `newest` (Recursive Backtracker), `oldest`, `middle`, `random` (Prim's). Followed by the weight (probability). | `{ random: 1 }` (Prim's) |
